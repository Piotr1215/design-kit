...
Run the Phase 1.5 feedback loop: read every `FEEDBACK.md` from completed proofs, propose deltas to `PLAN.md` and (if present) `SCHEMA.md`, get user confirmation, write a `.phase-1.5-complete` marker so `/design-kit:integration-tasks` will run.
...
- **let me edit** → write the proposed deltas to `$SPEC_DIR/REPLAN-PROPOSAL.md` and stop. The user edits, then re-runs `/design-kit:replan-after-research` to apply
...
```markdown
# Phase 1.5 marker — generated by /design-kit:replan-after-research
...
# Full digest below — useful as audit trail for /design-kit:integration-tasks
---
...
The marker is the gate `/design-kit:integration-tasks` will check. Re-running this command overwrites it.
...
❌ Generating new TASK-P1-* files — that's `/design-kit:research-tasks`'s job, and only after the user explicitly asks for refinement
...
❌ Skipping the marker write — without it, `/design-kit:integration-tasks` will refuse to run
...
- Before `/design-kit:integration-tasks` if it's been more than a few days since the last replan
...
- Run `/design-kit:integration-tasks` to generate Phase 2 tasks (the gate will now pass)
- Or run `/design-kit:status` for a state-machine view of where the project stands
